...
class RBloomFilter {
 public:
  ...
  : max_probes_(max_probes), bits_per_key_(bits_per_key), statistics_(stats) {
    ...
    printf("RBloomFilter size: %.3f MB\n", (double)filter_bytes/1024/1024);
  }
...
const HiBloomFilter* NewHiBloomFilter(int bits_per_key, int bits_per_level, int levels, int exam_suffix_bits, int num_keys, Statistics *stats) {
  return new HiBloomFilter(bits_per_key, bits_per_level, levels, exam_suffix_bits, num_keys, stats);
}

const RBloomFilter* NewRBloomFilter(int bits_per_key, int max_probes, int num_keys, Statistics *stats) {
