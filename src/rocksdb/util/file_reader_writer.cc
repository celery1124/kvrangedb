...
Status WritableFileWriter::Append(const Slice& data) {
  ...
  // chunks
  if (false && (use_direct_io() || (buf_.Capacity() >= left))) {
    while (left > 0) {
      size_t appended = buf_.Append(src, left);
      left -= appended;
      src += appended;
      // Mian, will not reach for kvssd
      if (left > 0) {
        s = Flush();
        if (!s.ok()) {
          break;
...
Status WritableFileWriter::Sync(bool use_fsync) {
  //Status s = Flush();
  Status s; // not flush before Sync (Mian)
  if (!s.ok()) {
    return s;
  }
...
Status NewAppendableFile(Env* env, const std::string& fname,
                       unique_ptr<WritableFile>* result,
                       const EnvOptions& options) {
  Status s = env->NewAppendableFile(fname, result, options);
  TEST_KILL_RANDOM("NewAppendableFile:0", rocksdb_kill_odds * REDUCE_ODDS2);
  return s;
}
