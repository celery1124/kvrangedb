...
void Insert(kvrangedb::DB* db, std::vector<uint64_t>* keys,
    ...
    kvrangedb::Slice s_value(value_buf, value_size);
...
void load(const std::string& key_path, const std::string& db_path, kvrangedb::DB** db,
	  kvrangedb::Options* options, int use_direct_io, uint64_t key_count, uint64_t value_size,
	...
	//     status = (*db)->Put(kvrangedb::WriteOptions(), s_key, s_value);
	//     if (!status.ok()) {
	// 	assert(false);
	//     }
...
void warmup(std::vector<uint64_t>* keys, uint64_t warmup_query_count, uint64_t key_range, kvrangedb::DB* db) {
    std::mt19937_64 e(0);
...
void benchPointQuery(kvrangedb::DB* db, kvrangedb::Options* options, int seed,
	...
	uint64_t key = dist(e);
    if (key%100 > empty_ratio) key = (*keys)[key%keys->size()];
	key = htobe64(key);
...
void benchOpenRangeQuery(kvrangedb::DB* db, kvrangedb::Options* options, int seed, uint64_t key_range,
    ...
    for (uint64_t i = 0; i < query_count; i++) {
    kvrangedb::ReadOptions rdopt;
    rdopt.scan_length = scan_length;
    kvrangedb::Iterator* it = db->NewIterator(rdopt);
...
void benchClosedRangeQuery(kvrangedb::DB* db, kvrangedb::Options* options,  int seed, uint64_t key_range,
	...
	kvrangedb::Slice s_upper_key(reinterpret_cast<const char*>(&upper_key), sizeof(upper_key));
...
int main(int argc, const char* argv[]) {
	...
	std::cout << "arg 11: point query empty ratio 1/1000\n";
	return -1;
    }
    ...
    int pq_empty_ratio = atoi(argv[11]);
    int packSize = 4000;
    ...
    options.rfMaxProbes = 1024;
    options.rfNumKeys = kKeyCount;
    if (filter_type == 1) {
